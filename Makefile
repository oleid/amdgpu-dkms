...
subdir-ccflags-y += -DOS_NAME_RHEL_7_4_5
...
ifeq ($(OS_NAME),"ubuntu")
ifeq ("oem","$(shell uname -r | awk -F '-' '{ print $$(NF) }')")
subdir-ccflags-y += -DUBUNTU_OEM_KERNEL
endif
endif
...
obj-m += amd/lib/ scheduler/ amd/amdgpu/ ttm/ amd/amdkcl/ amd/amdkfd/
